...
class DecentralizedCoordinator(nn.Module):         # decentralized "global" coordinator, as its dynamically elected 
    ...
    def forward(self, x: torch.Tensor, edge_index: Optional[torch.Tensor]) -> torch.Tensor:
        ...
        (Corrected)
        Args:
            x (torch.Tensor): The feature vectors of all agents in the communication graph. Shape: [num_nodes, hidden_dim]
        ...
        if edge_index is None or edge_index.numel() == 0:
            # If no communication, every agent is its own leader but has no squad to report on.
            # Return zero context as there's no squad information to share.
            return torch.zeros(num_nodes, self.context_dim, device=device)

        row, col = edge_index

        # --- Part 1: Leader Election (Decentralized & Corrected) ---
        leader_logits = self.leader_election_head(x)

        # Include self-loops to ensure every agent is a candidate for its own leader
        ...
        # The source tensor for scatter_max contains the logits of all potential leaders for all squads
        src_logits = leader_logits[extended_edge_index[0]].squeeze(-1)
        
        # The index tensor groups these logits by the agent they are a neighbor of
        target_node_idx = extended_edge_index[1]

        # Perform scatter_max to find the max logit and its index *within the src tensor* for each squad
        max_logits_per_squad, argmax_indices_in_src = scatter_max(
            src=src_logits,
        ...
        # --- CRITICAL FIX: Map the relative argmax back to the global node index ---
        # The `argmax_indices_in_src` are indices into `src_logits`. The actual node IDs that
        # correspond to these logits are stored in `extended_edge_index[0]`.
        leader_node_indices_for_squad = extended_edge_index[0][argmax_indices_in_src]
        
        # --- Handle isolated nodes robustly ---
        # An isolated node will have a max_logit of -inf (the fill_value of scatter_max).
        # We make these nodes their own leaders.
        is_isolated_mask = torch.isneginf(max_logits_per_squad)
        if is_isolated_mask.any():
            leader_node_indices_for_squad[is_isolated_mask] = torch.arange(num_nodes, device=device)[is_isolated_mask]
        ...
        # This indexing is now safe because `leader_node_indices_for_squad` contains valid global indices.
...
class TrailManager:
    """
    Manages all trail memories for a single SharedActorPolicy using a spatial hash grid
    for efficient O(1) proximity queries.
    """
    def __init__(self, cell_size: float, device: torch.device):
        self.cell_size = cell_size
        self.device = device
        self.grid: Dict[Tuple[int, int], List[int]] = {}
        self.trail_memories: Optional[nn.ModuleList] = None

    def set_memories(self, trail_memories_module_list: nn.ModuleList):
        """Links the manager to the policy's actual trail memory modules."""
        self.trail_memories = trail_memories_module_list

    def _get_cell_coords(self, world_pos: np.ndarray) -> Tuple[int, int]:
        """Hashes a world position to grid cell coordinates."""
        return (math.floor(world_pos[0] / self.cell_size),
                math.floor(world_pos[1] / self.cell_size))

    def rebuild_grid(self):
        """Clears and rebuilds the spatial grid from the current trail memory states."""
        self.grid.clear()
        if self.trail_memories is None:
            return

        for i, memory_module in enumerate(self.trail_memories):
            # Only add trails with non-zero intensity to the grid
            if memory_module.intensity > 1e-6:
                pos_np = memory_module.position.cpu().numpy()
                cell_coords = self._get_cell_coords(pos_np)
                if cell_coords not in self.grid:
                    self.grid[cell_coords] = []
                self.grid[cell_coords].append(i)

    def query_nearby_trails(self, agent_pos_np: np.ndarray, query_radius: float) -> Optional[torch.Tensor]:
        """
        (V2 - Vectorized) Efficiently finds all trail vectors within a radius of a given position.
        - Gathers all active trail data into tensors.
        - Uses torch.cdist for a single, fast distance calculation.
        - Vectorized filtering and intensity application.

        Returns:
            A tensor of nearby trail vectors, or None if no trails are found.
        """
        if not self.trail_memories:
            return None

        # 1. Gather all active trail data into tensors
        active_indices = [i for i, mem in enumerate(self.trail_memories) if mem.intensity > 1e-6]
        if not active_indices:
            return None

        trail_vectors = torch.stack([self.trail_memories[i].memory_vector for i in active_indices])
        trail_positions = torch.stack([self.trail_memories[i].position for i in active_indices])
        trail_intensities = torch.tensor([self.trail_memories[i].intensity for i in active_indices], device=self.device)

        # 2. Perform a single, batched distance calculation
        agent_pos_t = torch.from_numpy(agent_pos_np).to(self.device)
        distances = torch.linalg.norm(trail_positions - agent_pos_t, dim=1)

        # 3. Filter trails that are within the query radius
        within_radius_mask = distances < query_radius
        if not torch.any(within_radius_mask):
            return None

        nearby_vectors = trail_vectors[within_radius_mask]
        nearby_intensities = trail_intensities[within_radius_mask]

        # 4. Apply intensity decay to the final set of vectors
        # Reshape intensities for broadcasting: (Num_Nearby, 1) to match (Num_Nearby, Dim)
        return nearby_vectors * nearby_intensities.unsqueeze(1)
...
class SharedActor(nn.Module):
    def __init__(self,
        ...
        # self.obs_fusion is removed as it's part of the new state_fusion_mlp

        # --- 2. GNN Components ---
        mem_edge_input_dim = 8 + ((MEM_NODE_FEATURE_DIM + 1) * 2)
        comm_edge_input_dim = 8 + (NODE_FEATURE_DIM * 2)
        self.mem_gnn_edge_mlp = MLP(input_dim=mem_edge_input_dim, output_dim=self.edge_feature_dim, hidden_dim=mem_gnn_hidden_dim)
        self.comm_edge_mlp = MLP(input_dim=comm_edge_input_dim, output_dim=self.edge_feature_dim, hidden_dim=comm_gnn_hidden_dim)

        self.gnn_node_encoder_mem = MLP(MEM_NODE_FEATURE_DIM + 1, mem_gnn_hidden_dim)
        self.gnn_node_encoder_comm = MLP(NODE_FEATURE_DIM, comm_gnn_hidden_dim)
        ...
        self.gnn_out_proj = MLP(mem_gnn_hidden_dim, hidden_dim)
        self.coord_input_proj = MLP(comm_gnn_hidden_dim, self.temporal_output_dim)

        # --- 3. Persistent Memory Map Encoder ---
        memory_map_input_channels = self.memory_map_channels
        ...
        # --- 4. Unified State Fusion (REFACTORED) ---
        # All four streams (self, raw_map, memory_map, graph) are fused.
        # Input: self(d_model) + raw_map(hidden_dim) + mem_map(hidden_dim) + mem_graph(hidden_dim)
        state_fusion_input_dim = d_model + hidden_dim + hidden_dim + hidden_dim
        self.state_fusion_mlp = MLP(state_fusion_input_dim, hidden_dim, hidden_dim=hidden_dim*2)
    ...
    def forward(self,
                ...
                graph: Optional[Data], # This is the unified foveated graph
                nearby_trail_data: Optional[torch.Tensor],
                hidden_state: Optional[torch.Tensor],
                return_contrastive_embedding: bool = False,
                **kwargs
                ) -> Tuple[Dict[str, torch.Tensor], torch.Tensor, Dict[str, torch.Tensor], Optional[torch.Tensor], Optional[torch.Tensor], Optional[torch.Tensor], Optional[torch.Tensor], Dict[str, torch.Tensor]]:
        ...
        # --- 1. Base Perception (Self, Raw Map) ---
        ...
        if graph is not None and graph.num_nodes > 0:
            graph_data = graph.to(current_device)
            if not hasattr(graph_data, 'batch') or graph_data.batch is None: 
                graph_data.batch = torch.zeros(graph_data.num_nodes, dtype=torch.long, device=current_device)
            node_feats = self.gnn_node_encoder_mem(graph_data.x)
            if hasattr(graph_data, 'edge_index') and graph_data.edge_index.numel() > 0:
                edge_attr_mem = create_graph_edge_features(graph_data, self.mem_gnn_edge_mlp, self.mem_connection_radius)
            ...
            projected_graph_feats = self.gnn_out_proj(pooled_graph_feats)
            if projected_graph_feats.shape[0] < B: 
                mem_graph_encoded[:projected_graph_feats.shape[0]] = projected_graph_feats
            else: 
                mem_graph_encoded = projected_graph_feats[:B]
        
        # --- 3. Unified State Fusion ---
        combined_state_features = torch.cat([self_emb, map_encoded, mem_map_encoded, mem_graph_encoded], dim=1)
        full_state_fused = self.state_fusion_mlp(combined_state_features)

        # --- 4. High-Level Temporal Processing (GRU/Memory) ---
        temporal_input = full_state_fused.unsqueeze(1)
        ...
        if self.use_global_context and self.global_coordinator is not None and graph is not None and graph.num_nodes > 1:
            # Rebuild a simple communication graph from the "live" agents in the unified graph
            is_live_agent_mask = (graph.x[:, NODE_FEATURE_MAP['node_type_encoded']] == NODE_TYPE['agent']) & \
                                 (graph.x[:, MEM_NODE_FEAT_IDX['certainty']] > 0.99)
            
            if is_live_agent_mask.sum() > 1:
                live_agent_indices = torch.where(is_live_agent_mask)[0]
                live_agent_pos = graph.pos[live_agent_indices]
                live_agent_batch = graph.batch[live_agent_indices]
                
                # Create a map from the sparse `live_agent_indices` to a dense `0, 1, 2...` range
                map_live_idx_to_dense = torch.full((graph.num_nodes,), -1, dtype=torch.long, device=device)
                map_live_idx_to_dense[live_agent_indices] = torch.arange(live_agent_indices.shape[0], device=device)

                # The coordinator reasons over the high-level temporal states, not raw features
                # We need to scatter the batch's temporal_out to the sparse live agents
                coord_input_features = torch.zeros(live_agent_indices.shape[0], self.temporal_output_dim, device=device)
                
                # Find which live agents are ego agents and get their batch index
                is_ego_in_live_mask = graph.x[live_agent_indices, NODE_FEATURE_MAP['is_ego']] > 0.5
                if is_ego_in_live_mask.any():
                    ego_batch_indices = live_agent_batch[is_ego_in_live_mask]
                    # Place the temporal_out of the batch agents into the correct sparse positions
                    coord_input_features[is_ego_in_live_mask] = temporal_out[ego_batch_indices]

                    comm_edge_index = radius_graph(live_agent_pos, r=self.obs_radius, batch=live_agent_batch, max_num_neighbors=self.max_neighbors_comm)
                    
                    if comm_edge_index.numel() > 0:
                        squad_context_sparse = self.global_coordinator(x=coord_input_features, edge_index=comm_edge_index)
                        # Gather the context for just the ego agents and scatter it back to the main batch
                        ego_squad_contexts = squad_context_sparse[is_ego_in_live_mask]
                        global_context_val = scatter(ego_squad_contexts, ego_batch_indices, dim=0, dim_size=B, reduce='mean')

        # --- 6. Write and Process Trail Memory using Attention ---
        ...
        if nearby_trail_data is not None and self.trail_attention is not None:
            max_neighbors = nearby_trail_data.shape[1] if nearby_trail_data.dim() == 3 else 0
            if max_neighbors > 0:
                key_padding_mask = (nearby_trail_data.sum(dim=-1) == 0) # Mask if trail vector is all zeros
                processed_trail_info = self.trail_attention(
                    agent_state_query=temporal_out,
                    nearby_trails=nearby_trail_data,
                    key_padding_mask=key_padding_mask
                )
        
        # --- 7. Final Gating, Role Selection, and Planning ---
        ...
        final_input_components = [temporal_out, weighted_role_emb_val, global_context_val, processed_trail_info]
        final_input = torch.cat(final_input_components, dim=1)
        ...
        # --- 8. Low-Level Controller (Action Head) ---
        llc_input_components = [full_state_fused, weighted_role_emb_val] # Use fused state instead of temporal for more direct reaction
        if z_plan is not None:
            llc_input_components.insert(1, z_plan) 
        llc_input = torch.cat(llc_input_components, dim=1)
...
class SharedActorPolicy(nn.Module):
    ...
    def evaluate_actions(self,
        ...
        """
        (V3 - CORRECTED) Evaluates actions for a training batch.
        - Calls the network with the correct 'graph' keyword argument.
        ...
        # --- FORWARD PASS THROUGH CORE NETWORK (CORRECTED CALL) ---
        (action_output, new_hidden, hybrid_role_output, predicted_next, 
         global_context, z_plan, contrastive_embedding, message_written) = self.network(
            obs=obs_batch,
            memory_map_obs=obs_batch.get('memory_map'),
            graph=obs_batch.get('graph'),
            nearby_trail_data=external_nearby_trail_data,
            hidden_state=temporal_memory_in,
            return_contrastive_embedding=return_contrastive_embedding
        )
        
        # --- Process outputs for SAC (unchanged) ---
        ...
        # --- Assemble final outputs into standardized format (unchanged) ---
    ...
    def to(self, device):
        ...
        for trail_memory in self.agent_trail_memories:
            trail_memory.to(device)
        if self.key_contrastive_proj_head is not None:
...
if __name__ == "__main__":
    ...
    import sys
    from torch_geometric.data import Data, Batch
    from constants import NODE_FEATURE_DIM, RAW_CH_COUNT, OCC_CH_COUNT, MEM_NODE_FEATURE_DIM, NODE_FEATURE_MAP, OBS_RADIUS, WIDTH, HEIGHT
    ...
    # --- Test Parameters ---
    B = 4; SELF_DIM = 24; RAW_MAP_CH = RAW_CH_COUNT
    GRID = 32; MEM_MAP_CH = OCC_CH_COUNT+1; SEMANTIC_DIM_TEST = 32
    ...
    def generate_dummy_obs(batch_size):
        ...
        for _ in range(batch_size):
            num_nodes = 5
            # Create nodes with the full memory feature dimension
            x_single = torch.zeros(num_nodes, MEM_NODE_FEATURE_DIM + 1, device=device)
            # Populate the base features
            x_single_raw = torch.randn(num_nodes, NODE_FEATURE_DIM, device=device)
            x_single[:, :NODE_FEATURE_DIM] = x_single_raw
            
            # Generate positions and edges for the graph
            pos_single = torch.rand(num_nodes, 2, device=device) * 1000
            edge_index_single = radius_graph(pos_single, r=TEST_OBS_RADIUS * 2, max_num_neighbors=16)

            env_graphs.append(Data(
                x=x_single,
                pos=pos_single,
                radii=torch.rand(num_nodes, device=device),
                is_ego=torch.tensor([True] + [False]*(num_nodes-1), device=device),
                edge_index=edge_index_single
            ))

        mem_graphs = [Data(x=torch.randn(10, MEM_NODE_FEATURE_DIM + 1, device=device), pos=torch.rand(10,2, device=device)*1000, radii=torch.rand(10, device=device), is_ego=torch.tensor([True] + [False]*9, device=device)) for _ in range(batch_size)]
        return {
...
if __name__ == "__main__":
    print("This is a test print from __main__ block.")