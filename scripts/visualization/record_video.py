...
from Swarm2d.policies.simple_policies.map_policy import SimpleCNNMapPolicy # Reuse architecture


def record_episode(checkpoint_path, output_filename, steps=500, scenario='combat', fps=30.0):
    print(f"--- Recording Video: {output_filename} ---")
    ...
    # Scenario Overrides (Match simple_rl_training_MAP_ONLY.py)
    if scenario == 'exploration':
        ...
        env_config.update({
            'num_resources': 120, # Match training hack
            'num_obstacles': 5,
            'width': 400,
            'height': 400,
        ...
        env_config.update({
            ...
            'width': 400,
            'height': 400,
            'hive_min_distance': 50.0,
            'agent_spawn_radius': 100.0,
        })
    ...
    policy = SimpleCNNMapPolicy(map_ch, map_sz, mem_ch, mem_sz, self_dim).to(device)
